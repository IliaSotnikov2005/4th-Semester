...
/// Performs substitude.
let rec substitude var substitutionTerm baseTerm =
    match baseTerm with
    | Variable v when v = var -> substitutionTerm
    | Application(t1, t2) -> 
        Application(substitude var substitutionTerm t1, substitude var substitutionTerm t2)
    | Abstraction(param, body) when param = var -> baseTerm
    | Abstraction(param, body) when isFree param substitutionTerm ->
        ...
        Abstraction(newVar, substitude var substitutionTerm newBody)
    | Abstraction(param, body) -> Abstraction(param, substitude var substitutionTerm body)
    | _ -> baseTerm
